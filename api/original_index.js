...
// Funzione per caricare il torrent su RealDebrid
async function uploadToRealDebrid(magnetLink) {
  ...
  try {
      ...
      return response.data.id; // Restituisce l'ID del torrent su RealDebrid
  } catch (error) {
      console.error("Errore nell'upload su RealDebrid:", error.message);
      throw new Error("Upload su RealDebrid fallito.");
  }
}

// Funzione per recuperare i link di streaming del file caricato
async function getStreamLink(torrentId) {
  ...
  try {
      ...
      const streamUrl = response.data.links[0]; // Link del primo file
      return streamUrl;
  } catch (error) {
      console.error("Errore nel recupero del link di streaming:", error.message);
      throw new Error("Recupero link fallito.");
  }
}
...
app
  ...
  .get("/manifest.json", (req, res) => {
    ...
    var json = {
      ...
      logo: "https://i.imgur.com/9bJ04g2.jpeg",
      resources: [
        {
          name: "stream",
  ...
  .get('/debrid/:magnet', async (req, res) => {
    const magnetLink = req.params.magnet;
    try {
      const directLink = await uploadTorrent(magnetLink);
      res.json({ stream: directLink });
    } catch (error) {
      res.status(500).json({ error: 'Errore durante il caricamento su RealDebrid' });
    }
  })
